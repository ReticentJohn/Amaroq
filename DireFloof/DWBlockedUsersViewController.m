...
- (IBAction)unblockDomain:(id)sender
{
    ...
    [[MSAppStore sharedStore] unblockMastodonInstance:selectedDomain withCompletion:^(BOOL success, NSError *error) {
        ...
        {
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Error", @"Error") message:[NSString stringWithFormat:@"%@ %li", NSLocalizedString(@"Failed to unblock domain with error:", @"Failed to block domain with error:"), (long)error.code] preferredStyle:UIAlertControllerStyleAlert];
